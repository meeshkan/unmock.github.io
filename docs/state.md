...
myapi.state(
  withCodes(200),
  responseBody({ address: ["name"]}).const("Carolyn"),
  responseBody({ address: ["zodiac", "ascendenet"]}).const("Libra")
);
```

We've overridden the body so that the value at the address `body.name` is `"Carolyn"` and `body.zodiac.ascendenet` is `"Libra"`.
